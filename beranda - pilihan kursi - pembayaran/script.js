/* =============================================
   LBus — Skrip Pemesanan Tiket Bus
   ============================================= */
   ...
   const BUS_KEY = "lbus_bus"; // nama PO bus yang dipilih di halaman beranda
   const DEFAULT_BUS_NAME = "Sinar Jaya";
   
   function formatRupiah(num) {
     return "Rp " + num.toLocaleString("id-ID");
   }
   
   /* ---------- Nama PO bus yang dipilih (dipakai lintas halaman) ---------- */
   function saveBusName(name) {
     try {
       localStorage.setItem(BUS_KEY, name);
     } catch (e) {
       /* abaikan jika gagal */
     }
   }
   ...
   function getBusName() {
     try {
       return localStorage.getItem(BUS_KEY) || DEFAULT_BUS_NAME;
     } catch (e) {
       return DEFAULT_BUS_NAME;
     }
   }
   
   /* ---------- Status pemesanan (dipakai lintas halaman) ---------- */
   ...
   /* Buka/tutup grup metode pembayaran */
   ...
   function initBookingSummary() {
     ...
     // Tampilkan nama PO bus sesuai yang dipilih user di halaman beranda
     const busNameEl = document.querySelector('.bus-name');
     if (busNameEl) {
       busNameEl.textContent = getBusName();
     }
   ...
   /* Buka/tutup checkbox pilih penumpang */
   ...
   /* ---------- Pencarian bus (khusus halaman beranda / pindex.html) ---------- */
   function initBusSearch() {
     const searchInput = document.getElementById("searchInput");
     if (!searchInput) return; // elemen hanya ada di halaman beranda, aman di halaman lain
   
     searchInput.addEventListener("keyup", function () {
       const kataKunci = searchInput.value.toLowerCase();
       const kartuBus = document.querySelectorAll(".bus-card");
   
       kartuBus.forEach(kartu => {
         const isiKartu = kartu.textContent.toLowerCase();
   
         if (isiKartu.includes(kataKunci)) {
   ...
   /* ---------- Tombol "Lihat Detail" di halaman beranda (pindex.html) ---------- */
   function initDetailButtons() {
     const tombolDetail = document.querySelectorAll(".detail-btn");
     if (!tombolDetail.length) return; // hanya ada di halaman beranda, aman di halaman lain
   
     tombolDetail.forEach(tombol => {
       tombol.addEventListener("click", () => {
         const kartu = tombol.closest(".bus-card");
         const namaBus = kartu?.dataset.busName || DEFAULT_BUS_NAME;
         saveBusName(namaBus);
